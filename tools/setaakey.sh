#!/bin/bash
# Prompts for an Anna's Archive member API key (input hidden) and POSTs it
# to the running sendtox4d daemon. The key never appears in shell history.
set -euo pipefail

PORT="${SENDTOX4_PORT:-47821}"
URL="http://127.0.0.1:${PORT}/settings/aa-key"
...
printf "Anna's Archive member key (input hidden): "
stty -echo
IFS= read -r KEY
stty echo
printf "\n"
